...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    const parsedYear = Number.parseInt(tags.year, 10);
    ...
    const publication: Publication = {
      ...
      details: cleanBibTeXString(tags.details || tags.content),
      detailsSource: cleanBibTeXString(tags.details_source),
      showContents: tags.show_contents === 'true' || tags.show_contents === 'yes',
      selected,
      preview,

      // Store original BibTeX (excluding custom fields)
      bibtex: reconstructBibTeX(entry, ['selected', 'preview', 'description', 'details', 'content', 'details_source', 'show_contents', 'keywords', 'code']),
    };
