...
import { BasePageConfig, PublicationPageConfig, TextPageConfig, CardPageConfig, HighlightItem } from '@/types/page';
import { getRuntimeI18nConfig } from '@/lib/i18n/config';

interface SectionConfig {
  id: string;
  type: 'markdown' | 'publications' | 'list' | 'highlights';
  title?: string;
  source?: string;
  filter?: string;
  limit?: number;
...
function processSections(sections: SectionConfig[], locale?: string): SectionConfig[] {
  return sections.map((section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'highlights': {
        const highlightsData = section.source ? getTomlContent<{ highlights: HighlightItem[] }>(section.source, locale) : null;
        return {
          ...section,
