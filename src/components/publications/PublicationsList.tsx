...
export default function PublicationsList({ config, publications, embedded = false, pageSlug = 'publications' }: PublicationsListProps) {
    ...
    const years = useMemo(() => {
        const uniqueYears = Array.from(new Set(publications.map(p => p.year).filter((year): year is number => typeof year === 'number')));
    ...
    return (
        ...
        >
            <div className="mb-8">
                ...
                {config.description && (
                    <p className={`${embedded ? "text-base" : "text-lg"} text-neutral-600 dark:text-neutral-400 max-w-2xl`}>
                        {config.description}
                    </p>
                )}
            </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-6">
                                ...
                                <div className="flex-grow">
                                    <h3 className={`${embedded ? "text-lg" : "text-xl"} font-semibold text-primary mb-2 leading-tight`}>
                                        <Link href={`/${pageSlug}/${pub.id}`} className="hover:text-accent">
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-auto">
                                        {pub.doi && (
                                            <ResourceBadge label="DOI" url={`https://doi.org/${pub.doi}`} />
                                        )}
                                        {pub.code && (
                                            <ResourceBadge label={messages.publications.code} url={pub.code} />
                                        )}
                                        {pub.url && <ResourceBadge label="Paper" url={pub.url} />}
                                        {pub.pdfUrl && <ResourceBadge label="PDF" url={pub.pdfUrl} />}
                                        <ResourceBadge label="Blog" url={`/${pageSlug}/${pub.id}`} internal />
                                        {pub.abstract && (
                                            <button
